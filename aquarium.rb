...
class Aquarium
  ...
  def initialize
    ...
    @start_crucians_quantity = config["config"]["crucians_quantity"]
    @start_pikes_quantity    = config["config"]["pikes_quantity"]
    @field_size = vertical_size * horizontal_size
    @reservoir =  Array.new(field_size).map!{Array.new}
    fish_run
  end
  ...
  def stroke
    result = Array.new(field_size).map!{Array.new}
    reservoir.each_with_index do |elem, index|
      elem.delete_if { |fish| fish.weight == 0 }
      if elem.size >= 2
        eat! elem
          reproduce elem
      end
  ...
  def randomizer index
    ...
    array[output[0]].each_with_index do | elem_2, i_2|
        output << i_2 if elem_2 == index
    end
    index_one = output[0]
    index_two = output[1]
    result = []
  ...
  def reproduce elem
    ...
    if crucian_male != 0 && crucian_female != 0
      elem << Crucian.new
    end
    if pike_male != 0 && pike_female != 0
      elem << Pike.new
    end
    nil
  end
